...
from typing import Iterator, List, Literal, Optional, Tuple, TypedDict, Unpack
...
class PlainTextInterface(DomainInterface):
    ...
    def display(self, message: str, **kwargs: Unpack[DisplaySettings]) -> None:
        ...
        if is_traceback and kwargs and kwargs.get("collapsible", False):
            message, _ = _extract_condensed_traceback(message)
...
class IPythonInterface(DomainInterface):
    @staticmethod
    def is_available() -> bool:
        """Check if running inside a Jupyter notebook where Markdown renders properly."""
        try:
    ...
    def prompt(self, prompt: str, **kwargs: Unpack[DisplaySettings]) -> Optional[str]:
        ...
        display("IPythonInterface does not support prompts")
